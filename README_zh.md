...
> "ETH 快到我的目标价了，到 $2,150 通知我——我要卖 3.5 个 ETH 换 HYPE。"

agent 在后台：

1. **注册**价格警报：`ETH >= $2,150`
2. **拉起**监控守护进程（如果没在跑）
3. **回复**你：*"收到，ETH 到 $2,150 通知你。"*

守护进程每 60 秒从 Binance 取价。几小时后，ETH 触及 $2,152：
...
资产:     ETH
条件:     >= $2,150
当前价:   $2,152.30 (来源: binance)
上下文:   卖 3.5 ETH 换 HYPE
```

agent 醒来，读取上下文，主动给你发消息：

> "ETH 刚到 $2,152。按你的计划——现在可以在 OKX 卖 3.5 个 ETH，拿约 $4K 买 HYPE。要我帮你走一遍操作流程吗？"

**不用盯盘，不用开浏览器。agent 替你看着市场。**

新闻盯盘同理——设关键词比如 `"SEC, 加密, 监管"`，agent 每 5 分钟扫一遍金十、华尔街见闻和主流 Crypto RSS，有匹配才通知你。
